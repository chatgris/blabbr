class SessionsController < ApplicationController
  ...
  def open_id_authentication(identity_url)
    authenticate_with_open_id(identity_url, :required => [ :email, 'http://schema.openid.net/contact/email' ]) do |result, identity_url, registration|
      if result.successful?
        email = registration['email'] || registration['http://schema.openid.net/contact/email']
